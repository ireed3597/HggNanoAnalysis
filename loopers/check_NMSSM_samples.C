...
int ScanChain( TChain *ch, string proc, int year, float scale_factor = 1, bool resonant = false ) {
    ...
    while ( (currentFile = (TFile*)fileIter.Next()) ) {
        ...
        for( unsigned int loop_event = 0; loop_event < tree->GetEntriesFast(); ++loop_event) {
			...
			//photon selection
				if ( Photon_pt().at(i) > pho_pt_cut  && Photon_mvaID().at(i) > pho_idmva_cut && Photon_electronVeto().at(i) > pho_eveto_cut && fabs(Photon_eta().at(i)) < pho_eta_cut  ) sel_phos.push_back(i);
			}

			if (sel_phos.size() < 2 ) continue;
			mgg = ( Photon_p4().at(sel_phos[0]) + Photon_p4().at(sel_phos[1]) ).M();
			...
			h_mgg->Fill( mgg );
			h_nTau->Fill( sel_taus.size() );
        } // Event loop
        delete file;
    } // File loop
    bar.finish();
	
	TCanvas *c = new TCanvas("c","mgg");
	h_mgg->Draw();
	c->SaveAs("/home/users/fsetti/public_html/HH2ggtautau/check/mgg.pdf");
	h_nTau->Draw();
	c->SaveAs("/home/users/fsetti/public_html/HH2ggtautau/check/nTau.pdf");
	f1->Write();
	f1->Close();
	return 0;
}
