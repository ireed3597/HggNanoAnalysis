...
int ScanChain( TChain *ch, string proc, string str_year, float scale_factor = 1, bool resonant = false ) {
  ...
  TString file_name = proc + "_10Mar2022_" +  str_year;
    ...
    while ( (currentFile = (TFile*)fileIter.Next()) ) {
      ...
      nt.Init(tree);
			...
			TFile* f1 = new TFile("outputs_UL/add_BDT_vars/" + proc + "/" + file_name + "_" + std::to_string(idx) + ".root", "RECREATE");
			H1(mgg, 60, 100 , 180 );
...
//						||	(	fabs(Photon_eta().at(i)) >  1.5 && Photon_r9().at(i) < 0.90 && Photon_r9().at(i) > 0.8 && Photon_sieie().at(i) < 0.035 && Photon_trkSumPtHollowConeDR03().at(i) < 6.0  && ( Photon_photonIso().at(i) - 0.13212*fixedGridRhoAll() ) < 4.0 )			/*pho_EE_lowR9 */ )
		...
		for(unsigned int i=0; i<nJet(); i++){
			if (Jet_pt().at(i) > jet_pt && fabs(Jet_eta().at(i)) < jet_eta && Jet_neEmEF().at(i) < jet_neEmEF && Jet_neHEF().at(i) < jet_neHEF && Jet_chHEF()[i] > jet_chHEF && Jet_chEmEF()[i] < jet_chEmEF && deltaR( Jet_p4().at(i) , Photon_p4().at(gHidx[0]) ) > jet_dR_pho && deltaR( Jet_p4().at(i) , Photon_p4().at(gHidx[1]) ) > jet_dR_pho ){

				int jet_const = (int)Jet_nConstituents().at(i) ;
				if ( jet_const <= jet_nConstituents ) continue;
				cout << jet_const << endl;
				//if ( Jet_nConstituents()[i] <= jet_nConstituents ) continue;
